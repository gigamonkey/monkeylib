...
(deftest pass ()
  (multiple-value-bind (ok passes failures)
      ...
      (ignore-results (test monkeylib-test-tests::always-pass))
    (check ok (= passes 1) (= failures 0))))

(deftest fail ()
  (multiple-value-bind (ok passes failures)
      (ignore-results (test monkeylib-test-tests::always-fail))
    (check (not ok) (= passes 0) (= failures 1))))

(deftest multiple-passes ()
  (multiple-value-bind (ok passes failures)
      (ignore-results (test monkeylib-test-tests::should-pass-n 10))
    (check ok (= passes 10) (= failures 0))))

(deftest multiple-failures ()
  (multiple-value-bind (ok passes failures)
      (ignore-results (test monkeylib-test-tests::should-fail-n 10))
    (check (not ok) (= passes 0) (= failures 10))))

(deftest mixed-results ()
  (multiple-value-bind (ok passes failures)
      (ignore-results
	(test monkeylib-test-tests::should-pass-n-fail-m 10 13))
    (check (not ok) (= passes 10) (= failures 13))))

(deftest test-test-package () 
  (multiple-value-bind (ok passes failures aborts)
      (let ((*debug* nil))
	(ignore-results (test-package :print t :summary nil :package :monkeylib-test-tests)))
    (check (not ok) (= passes 2) (= failures 2) (= aborts 1))))

(deftest test-expect ()
  (multiple-value-bind (ok passes failures)
      (ignore-results (test monkeylib-test-tests::expect/pass))
    (check ok (= passes 1) (= failures 0)))
  (multiple-value-bind (ok passes failures)
      (ignore-results (test monkeylib-test-tests::expect/fail))
    (check (not ok) (= passes 0) (= failures 1)))
  (multiple-value-bind (ok passes failures aborts)
      (let ((*debug* nil))
	(ignore-results (test monkeylib-test-tests::expect/abort)))
